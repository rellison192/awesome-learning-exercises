...
concepts in our daily JavaScript use. 
...
🚨 Do _NOT_ change the definition of the 'Robot' function.
`;
...
function Robot(name) {
  ...
  // Each robot has its own quote method which
...
🛠️   * the speak function MUST BE USED.
...
🚨 Do _NOT_ change the definition of the 'speak' function.
`
...
1) Arrow functions have their context bound lexically! 
  ...
  the context of the function it was defined in (which can be the global scope).
...
function Notifier() {
  ...
  const trigger = function(event, data) {
    ...
    // This is an old trick of ES5 to save the current context (this) into a different variable,
